    ...
    <section>
        <div className="max-w-5xl flex flex-col mx-auto">
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 relative">
                {
                    allBlogs.map(blog => (
                        <div key={blog.img} className="w-80 flex justify-center items-center flex-col p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.05),0_12px_30px_rgba(0,0,0,0.18)] rounded-2xl hover:scale-105 duration-300 cursor-pointer" onClick={() => window.location.href=`/blogs/${blog.id}`}>
